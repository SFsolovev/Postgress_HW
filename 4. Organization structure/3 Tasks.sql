...
WITH RECURSIVE Subordinates AS (
    SELECT e.EmployeeID, e.Name, e.ManagerID, e.DepartmentID, e.RoleID, e.EmployeeID AS RootManager
      FROM Employees e
    UNION ALL
    SELECT e.EmployeeID, e.Name, e.ManagerID, e.DepartmentID, e.RoleID, s.RootManager
      FROM Employees e
           JOIN Subordinates s ON e.ManagerID = s.EmployeeID)
SELECT e.EmployeeID, e.Name AS EmployeeName, e.ManagerID, d.DepartmentName, r.RoleName,
       STRING_AGG(DISTINCT p.ProjectName, ', ') AS ProjectNames,
       STRING_AGG(DISTINCT t.TaskName, ', ') AS TaskNames,
       COUNT(DISTINCT s.EmployeeID) - 1 AS TotalSubordinates
  FROM Employees e
       JOIN Roles r ON e.RoleID = r.RoleID
       LEFT JOIN Departments d  ON e.DepartmentID = d.DepartmentID
       LEFT JOIN Projects p     ON d.DepartmentID = p.DepartmentID
       LEFT JOIN Tasks t        ON e.EmployeeID = t.AssignedTo
       LEFT JOIN Subordinates s ON e.EmployeeID = s.RootManager
 WHERE r.RoleName = 'Менеджер'
 GROUP BY e.EmployeeID, e.Name, e.ManagerID, d.DepartmentName, r.RoleName
HAVING COUNT(DISTINCT s.EmployeeID) - 1 > 0
 ORDER BY e.Name;
